...
import { getTopRated } from "@/lib/dummyjson"
import { ProductCard } from "@/components/home/product-card"
import { Carousel } from "@/components/home/carousel"
import { ViewAllCard } from "@/components/home/view-all-card"

async function TopRated() {
  const products = await getTopRated(16)

  return (
    <section className="bg-background">
      <div className="mx-auto max-w-[1600px] px-4 pt-6 pb-4 sm:px-6 md:pt-10 md:pb-7 lg:px-10 xl:px-14">
        <div className="mb-5 flex items-start justify-between sm:mb-8 sm:items-end">
          <Link href="/shop?sort=rating-desc" className="group">
            <h2 className="text-2xl font-bold tracking-tight text-foreground group-hover:underline sm:text-3xl">
              Top Rated
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Customer favorites, rated highest by shoppers like you
            </p>
          </Link>
          <Link
        ...
        <Carousel>
          ...
          <ViewAllCard href="/shop?sort=rating-desc" />
        </Carousel>
      </div>
    </section>
  )
